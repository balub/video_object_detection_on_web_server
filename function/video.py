...
def video(detection):
    ...
    output_pq = PriorityQueue(maxsize=3*detection.get_queue_size())
    pool = Pool(detection.get_num_workers(), worker, (input_q,output_q))
    
    # created a threaded video stream and start the FPS counter
    # vs = cv2.VideoCapture("inputs/{}".format(detection.get_input_videos()))
    vs = cv2.VideoCapture(detection.get_input_videos())
    fps = FPS().start()

    # Define the codec and create VideoWriter object
    if detection.get_output():
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter(detection.get_output_path(),
